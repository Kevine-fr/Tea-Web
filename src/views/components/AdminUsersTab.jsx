...
.user-avatar { width:34px; height:34px; border-radius:50%; display:flex; align-items:center; justify-content:center; font-size:.75rem; font-weight:700; flex-shrink:0; font-family:'Lato',sans-serif; }
`
...
function PaginationU({ meta, page, onPage }) {
...
export function AdminUsersTab({ search = '', currentUserId, refreshKey }) {
  ...
  async function load(p = 1) {
    setLoading(true)
    try {
      const res = await adminApi.users({ page:p, search:search||undefined, role:roleFilter==='all'?undefined:roleFilter })
      setUsers(res.data ?? []); setMeta(res.meta ?? {})
    } catch { toast.error('Impossible de charger les utilisateurs.') }
    finally { setLoading(false) }
  }

  useEffect(() => { setPage(1); load(1) }, [search, roleFilter, refreshKey])
  ...
  async function handleUpdate(vals) {
    try {
      ...
      if (vals.role !== currentRole) await adminApi.updateRole(modal.user.id, vals.role)
  ...
  async function handleDelete() {
    ...
    try { await adminApi.deleteUser(modal.user.id); toast.success('Utilisateur supprimé.'); setModal(null); load(page) }
    catch (err) { toast.error(err.response?.data?.message || 'Erreur.') }
  }
  ...
  return (
    <>
      ...
      <div style={{ padding:'1.75rem' }}>
        ...
        {loading
          ...
          : users.length === 0
            ...
            : <div className="card" style={{ overflow:'auto' }}>
                <table className="tbl">
                  <thead><tr><th>Utilisateur</th><th>Mail</th><th>Rôle</th><th>Inscrit le</th><th>Actions</th></tr></thead>
                  <tbody>
                    {users.map((u,i) => {
                        ...
                        <tr key={u.id} className="user-row" style={{ animationDelay:`${i*.04}s` }}>
                          ...
                          <td>
                            <div style={{ display:'flex', gap:'.3rem' }}>
                              <button className="btn btn-orange" style={{ padding:'.25rem .7rem', fontSize:'.75rem' }}
                                onClick={() => setModal({ type:'edit', user:u })}>Modifier</button>
                              {!isMe && (
                                <button className="btn btn-outline"
                                  style={{ padding:'.25rem .65rem', fontSize:'.78rem', color:'var(--error)', borderColor:'var(--error)' }}
                                  onClick={() => setModal({ type:'delete', user:u })}>🗑</button>
                              )}
                            </div>
                          </td>
                        </tr>
        ...
        <PaginationU meta={meta} page={page} onPage={goPage} />
      </div>
